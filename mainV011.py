...
class Outpost(object):
    ...
    def information(self, allInfo=True):
        """ Description of self for infoo command. """
        if allInfo:
            result = "***==== {0} Outpost Info ====***\nOwner: {7}\nLevel: {1}\nType: {2}\nLocation: {3}\nUpgrade Cost: {4}\nTroops: {5}\\{6}\n".format(
            self.name, self.level, outpostTypes[self.typ], "{} Rim".format(["Core","Inner","Middle","Outer","Deep"][self.loc]),
            self.world.stats["oc"][self.level-1], self.troops, self.pop, self.owner
            )
            if self.typ == "Steel":
                result += "Steel Production: {0}\n".format(self.bounty)
            elif self.typ == "Gems":
                result += "Gems Production: {0} to {1}\n".format(self.bounty // 3, self.bounty + self.bonus)
        else:
            result = "***==== {0} Outpost Info ====***\nOwner: {5}\nLevel: {1}\nType: {2}\nLocation: {3}\nTroop Capacity: {4}\n".format(
            self.name, self.level, outpostTypes[self.typ], "{} Rim".format(["Core","Inner","Middle","Outer","Deep"][self.loc]),self.pop, self.owner
            )
        return result
    ...
    def upgrade(self):
        ...
        if self.level < 12:
            if self.ownerInfo["resources"]["Steel"] >= self.world.stats["oc"][self.level-1]:
                ...
                self.level += 1
                self.updateStats()
                return "Upgrade successful. You have {} Steel left.".format(self.ownerInfo["resources"]["Steel"])
            return "You need {0} more Steel to upgrade {1}.".format(
                self.world.stats["oc"][self.level-1]-self.ownerInfo["resources"]["Steel"], self.name
                )
        return "Upgrade failed. Outpost is at maximum level."
    
    def reapBounty(self, ticks):
        """
        Gives the owner the bounty and the added bonus of the owner.
        Simulates collect after certain number of ticks.
        """
        
        if self.typ == "Gems":
            self.ownerInfo["resources"][self.typ] += (random.randint(self.bounty // 3, self.bounty + self.bonus)) * ticks
        elif self.typ == "Steel":
            self.ownerInfo["resources"][self.typ] += (self.bounty + self.bonus) * ticks
...
class Player(object):
    ...
    def __init__(self, name, world, saved=False):
        ...
        Attributes:
            ...
            offline     = <int> Ticks until offline collection limit is applied (starts at 48, 1\2 day)
        """
    ...
    def information(self, allInfo=True):
        """ Shows information of player for infof command. Need to include resource production stats later. """
        result = "***==== {0} Faction Info ====***\n".format(self.name)
        result += "Outposts: {}\n".format(
            [self.world.info["outposts"][o].describe() for o in self.outposts]
            )
        if allInfo:
            result += "Gems: {}\n".format(self.resources["Gems"])
            result += "Steel: {}\n".format(self.resources["Steel"])
            result += "Troops: {0}/{1}\n".format(self.resources["Troops"],self.resources["Pop"])
            result += "Energy: {}\n".format(self.energy)
        else:
            result += "Troop Capacity: {}\n".format(self.resources["Pop"])
        return result
    ...
    def load(self):
        ...
        self.offline = s["offline"]

    def reapBounty(self, ticks):
        """
        Reap bounties from all outposts owned and count population.
        Simulates number of ticks, limits collection and applies basic income.
        """

        if ticks > self.offline:
            ticks = int(self.offline)
            self.offline = 0
        else:
            self.offline -= ticks
        ...
        # Implements basic income if player has no outposts
        if len(self.outposts) == 0:
            self.resources["Steel"] += self.world.stats["minSteel"] * ticks
            self.resources["Gems"] += self.world.stats["minGems"] * ticks
        
        for o in self.outposts:
            self.world.info["outposts"][o].reapBounty(ticks)
    ...
    def strike(self, troops, source, target):
        ...
        if not(okint(troops)) or int(troops) < 1:
        ...
        while True:
            # Damage can't be more than the number of troops in the opposing party
            damageAtk = min(min(math.ceil(attackers * atkMultiplier * (random.randint(25, 35) / 100)), oatk), defenders)
            damageDef = min(min(math.ceil(defenders * defMultiplier * (random.randint(25, 35) / 100)), odef), attackers)
            ...
            if attackers < retreat:
                ...
                self.resources["Troops"] -= attackers - oatk
                enemy.resources["Troops"] -= defenders - odef

                self.save()
                
                return "Strike failed. Your troops suffered {0} casualities and retreated. Target outpost has {1} troop(s) left. You have {2} Energy left.".format(
                    oatk - attackers, self.world.info["outposts"][target].troops, self.energy
                    )
            
            elif defenders <= 0:
                if attackers <= self.world.info["outposts"][target].pop:
                    self.world.info["outposts"][target].troops = attackers
                else:
                    # If the outpost can't hold all remaining attackers, send leftover back to source
                    self.world.info["outposts"][target].troops = self.world.info["outposts"][target].pop
                    self.world.info["outposts"][source].troops += attackers
                ...
                return "Strike successful. You have taken control of target outpost {0} with {1} troop(s) left. You have {2} Energy left.".format(
    ...
    def train(self, troops, outpost):
        ...
        if self.world.info["outposts"][outpost].troops + troops < self.world.info["outposts"][outpost].pop + 1:
            ...
            if self.resources["Gems"] - cost > -1:
                ...
                self.save()
                return "Training successful. {0} now has {1} troop(s). You have {2} Gems remaining.".format(
                    outpost, self.world.info["outposts"][outpost].troops, self.resources["Gems"]
                    )
            return "Training failed. You need {0} more Gems to train {1} troop(s).".format(
                cost - self.resources["Gems"], troops
                )
        return "Training failed. The population of {0} is insufficient for the deployment of {1} troop(s).".format(
            outpost, troops
            )
        ...
        if cost <= self.energy:
            ...
            if d.troops + troops < d.pop + 1:
                ...
                if s.troops + 1 > troops:
                    ...
                    return "Reinforcement successful. {0} now has {1} troop(s). {2} now has {3} troop(s). You have {4} energy left.".format(
    ...
    def buildOutpost(self, name, typ, loc):
        if okint(loc) and 0 <= int(loc) <= 4:
            ...
            return "Construction failed. The types of outposts are {}. See '$tut outposts' for more info.".format(
                "('Forge', 'Ark', 'Harvester')"
                )
...
class newPlayer(Player):
    ...
    def __init__(self, name, world):
        """
        Creates a completely new Player object.
        """
...
class World(json_help.File):
    ...
    def __init__(self, path, resourcePath="resources\\stats.json"):
        ...
        Attributes: 
            ...
            time      = <File> Storing information on time
            tickTime  = <int> Minutes between two ticks
        """

        super(World, self).__init__(path)

        self.stats = json_help.File(resourcePath).save
        self.time = json_help.File("saves\\time.json")
        self.tickTime = 15
    ...
    def time_str(self):
        return datetime.datetime.now().strftime("%Y/%d/%m %I:%M %p (CA)")

    def update_time(self, update_world=True):
        """ Updates time in time.json file.

        Time is stored as [days since epoch, seconds since beginning of day].
        Compares current time (naive UTC) to time stored in time.json.

        update_world can be False to clean time.json without updating world objects.
        """

        c = datetime.datetime.utcnow() - datetime.datetime(1,1,1)
        self.time.save["Current"] = repr(c)[19:-1].split(", ")[:2]

        #print(self.time.save["Current"])
        #print(self.time.save["Next"])

        ticks = 0
        if self.time.save["Current"] == self.time.save["Next"]:
            self.time.save["Next"][1] = str(int(self.time.save["Next"][1]) + self.tickTime * 60)
            if int(self.time.save["Next"][1]) >= 86400:
                # The tick goes into the next day
                self.time.save["Next"][1] = str(int(self.time.save["Next"][1]) - 86400)
                self.time.save["Next"][0] = str(int(self.time.save["Next"][0]) + 1)
            ticks = 1
        elif [int(x) for x in self.time.save["Current"]] > [int(y) for y in self.time.save["Next"]]:
            daysElapsed = int(self.time.save["Current"][0]) - int(self.time.save["Next"][0])
            secsElapsed = int(self.time.save["Current"][1]) - int(self.time.save["Next"][1])

            #print(daysElapsed)
            #print(secsElapsed)

            ticks = secsElapsed // (self.tickTime * 60)
            ticks += (daysElapsed * 86400) // (self.tickTime * 60)

            self.time.save["Next"][1] = (int(self.time.save["Current"][1]) // (self.tickTime * 60)) * (self.tickTime * 60) + (self.tickTime * 60)
            self.time.save["Next"][0] = int(self.time.save["Current"][0])
            if self.time.save["Next"][1] >= 86400:
                self.time.save["Next"][1] = int(self.time.save["Next"][1]) - 86400
                self.time.save["Next"][0] = int(self.time.save["Next"][0]) + 1

            self.time.save["Next"][0] = str(self.time.save["Next"][0])
            self.time.save["Next"][1] = str(self.time.save["Next"][1])
            
        #print("Ticks to catch up: {}".format(ticks))
        if update_world:
            self.update(ticks)
            self.save_all()
            
        self.time.write_save()
