    ...
    def information(self, allInfo=True):
        ...
        if allInfo:
            ...
            if self.typ == "Steel":
                ...
                result += "Gems Production: {0} to {1}\n".format(round(self.bounty / 3), self.bounty + self.bonus)
        else:
            result = "***==== {0} Outpost Info ====***\nOwner: {5}\nLevel: {1}\nType: {2}\nLocation: {3}\nTroop Capacity: {4}\n".format(
...
class World(json_help.File):
    ...
    def update_time(self):
        ...
        current = datetime.datetime(1,1,1).now()
        self.time.save["Current"] = current.minute + current.hour * 60

        ticks = 0 
        if self.time.save["Current"] == self.time.save["Next"]:
            self.time.save["Next"] = (self.time.save["Next"] + self.tickTime) % 1440
            ticks = 1
        elif self.time.save["Current"] > self.time.save["Next"]:
            ticks = ((self.time.save["Current"] // self.tickTime * self.tickTime) - self.time.save["Next"]) // self.tickTime + 1
            self.time.save["Next"] = (self.time.save["Current"] // self.tickTime) * self.tickTime + self.tickTime
