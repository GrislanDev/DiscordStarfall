...
class Outpost(object):
    ...
    def upgrade(self):
        ...
        """
        if self.level < 12:
            if self.ownerInfo["resources"][self.typ] <= self.world.stats["oc"]:
                self.level += 1
                self.ownerInfo["resources"][self.typ] -= self.world.stats["oc"]
                self.updateStats()
            return "Upgrade failed. You can't afford the cost of {} Steel.".format(self.world.stats["oc"])
